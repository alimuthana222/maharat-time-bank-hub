...
export function BookingForm({ providerId, serviceId, hourlyRate, onBookingCreated }: BookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("تم إنشاء طلب الحجز بنجاح");
    } catch (error: any) {
  ...
  const handlePaymentSuccess = () => {
    ...
    setMessage("");
    onBookingCreated?.();
    toast.success("تم الدفع وتأكيد الحجز بنجاح!");
  };
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded-lg">
        ...
        <div className="p-3 bg-gray-50 rounded-lg">
          ...
          <div className="flex justify-between items-center">
            ...
            <span>{hourlyRate.toLocaleString()} د.ع</span>
          </div>
      ...
      {showPayment && bookingId && (
        <PaymentDialog
          ...
          amount={totalCost}
          description={`دفع حجز خدمة - ${totalHours} ساعة`}
